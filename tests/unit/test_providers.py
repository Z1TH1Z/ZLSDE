...
class TestLocalProvider:
    ...
    def test_local_provider_initialization(self):
        ...
        with (
            patch("zlsde.providers.local_provider.AutoTokenizer"),
...
class TestProviderFactory:
    ...
    def test_create_providers_api_with_keys(self):
        ...
        with (
            patch.dict(
                "os.environ",
                {"GROQ_API_KEY": "test_groq_key", "MISTRAL_API_KEY": "test_mistral_key"},
            ),
            ...
            # Setup mocks
            for mock_provider in [mock_groq, mock_mistral]:
                mock_instance = Mock()
                mock_instance.is_available.return_value = True
            ...
            # api mode uses only API providers — no local fallback
...
class TestProviderConfig:
    ...
    def test_provider_config_validation_valid(self):
        ...
        config = ProviderConfig(provider_type="api", api_providers=["groq", "mistral"], timeout=30)
    ...
    def test_provider_config_validation_invalid_provider(self):
        ...
        config = ProviderConfig(provider_type="api", api_providers=["invalid_provider"])
