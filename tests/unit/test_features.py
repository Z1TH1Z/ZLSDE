...
from zlsde.models.data_models import (
    PipelineConfig, DataSource, ProviderConfig, Label,
    LabeledDataItem, RawDataItem,
...
class TestProvenanceTracker:
    ...
    def test_add_explanation(self):
        ...
        report = tracker.generate_report()
        assert report.provenance_records[0].explanation == "These samples are about sports activities"
...
class TestSemanticValidation:
    ...
    def test_no_flags_on_clean_data(self):
        ...
        for i in range(n):
            ...
            items.append(LabeledDataItem(
                id=f"item_{i}", content=f"text {i}", embedding=emb,
                label=f"label_{cid}", cluster_id=cid, confidence=0.9,
