...
from zlsde.layers.label_generation import PseudoLabelGenerator, create_label_generator
...
class TestLabelGeneratorIntegration:
    ...
    def test_create_label_generator_with_local_config(self):
        ...
        config = ProviderConfig(provider_type="local", local_model="google/flan-t5-base")
    ...
    def test_create_label_generator_with_pipeline_config(self):
        ...
        config = PipelineConfig(
            ...
            provider_config=ProviderConfig(
                provider_type="local", local_model="google/flan-t5-base"
            ),
        )
...
class TestConfigLoaderIntegration:
    ...
    def test_load_config_with_provider_config(self):
        ...
        config_dict = {
            "data": {"sources": [{"type": "csv", "path": "test.csv"}], "modality": "text"},
            "labeling": {
                "provider_config": {
                    "provider_type": "api",
    ...
    def test_load_config_backward_compatibility(self):
        ...
        config_dict = {
            "data": {"sources": [{"type": "csv", "path": "test.csv"}], "modality": "text"},
            "labeling": {"llm_model": "google/flan-t5-large", "use_llm": True},
        }
...
class TestEndToEndIntegration:
    ...
    @pytest.mark.skipif(
        not os.getenv("GROQ_API_KEY"), reason="GROQ_API_KEY not set - skipping live API test"
    )
    ...
    @pytest.mark.skipif(
        not os.getenv("MISTRAL_API_KEY"), reason="MISTRAL_API_KEY not set - skipping live API test"
    )
    ...
    def test_live_api_call_openrouter(self):
        ...
        provider = OpenRouterProvider(
            api_key=api_key, model="mistralai/mixtral-8x7b-instruct", timeout=30
        )
