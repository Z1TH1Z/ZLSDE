    ...
    with gr.Blocks(title="ZLSDE - Zero-Label Dataset Engine", theme=gr.themes.Soft()) as app:
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                # File upload
                input_file = gr.File(
                    label="Upload Data File", file_types=[".csv", ".json", ".txt"], type="filepath"
                )
                ...
                with gr.Accordion("🔄 Training Settings", open=False):
                    ...
                    random_seed = gr.Number(
                        value=42, label="Random Seed", info="For reproducible results"
                    )
...
def main():
    ...
    app = create_ui()
    app.launch(
        server_name="127.0.0.1", server_port=7860, share=False, show_error=True, inbrowser=True
    )
