    ...
    def create_providers(config, device: str = "cpu") -> List[LLMProvider]:
        ...
        if config.provider_type == "api":
            ...
            for provider_name in config.api_providers:
                provider = ProviderFactory._create_api_provider(provider_name, config)
            ...
            # No local fallback when provider_type is "api" — fail explicitly if all API providers are down
