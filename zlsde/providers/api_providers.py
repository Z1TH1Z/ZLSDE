...
class GroqProvider(LLMProvider):
    ...
    def generate_label(self, prompt: str, max_tokens: int = 20) -> str:
        ...
        try:
            ...
            raise TimeoutError(
                f"Request timeout after {self.timeout}s for {self.get_provider_name()}"
            )
    ...
    def _parse_response(self, response_json: dict) -> str:
        ...
        try:
            ...
            raise InvalidResponseError(
                f"Invalid response format from {self.get_provider_name()}: {e}"
            )
...
class MistralProvider(LLMProvider):
    ...
    def __init__(self, api_key: str, model: str = "mistral-small-latest", timeout: int = 30):
...
class OpenRouterProvider(LLMProvider):
    ...
    def __init__(
        self, api_key: str, model: str = "mistralai/mixtral-8x7b-instruct", timeout: int = 30
    ):
