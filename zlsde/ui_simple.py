...
def run_pipeline(
    ...
    try:
        ...
        if provider_type == "API Services":
            ...
            provider_config = ProviderConfig(provider_type="local", local_model=llm_model)
...
with gr.Blocks(title="ZLSDE - Zero-Label Dataset Engine") as app:
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            # File upload
            input_file = gr.File(label="Upload Data File", file_types=[".csv", ".json", ".txt"])
            ...
            with gr.Accordion("⚙️ Basic Settings", open=True):
                modality = gr.Dropdown(
                    choices=["text", "image", "multimodal"], value="text", label="Data Modality"
                )

                output_format = gr.Dropdown(
                    choices=["csv", "json", "parquet"], value="csv", label="Output Format"
                )
            ...
            with gr.Accordion("🔍 Clustering Settings", open=False):
                clustering_method = gr.Dropdown(
                    choices=["auto", "kmeans", "spectral"], value="auto", label="Clustering Method"
                )

                min_cluster_size = gr.Slider(
                    minimum=2, maximum=50, value=5, step=1, label="Minimum Cluster Size"
                )

            # Label generation settings
            with gr.Accordion("🏷️ Label Generation Settings", open=False):
                use_llm = gr.Checkbox(value=True, label="Use LLM for Label Generation")
...
```
                    ...
                    api_timeout = gr.Slider(
                        minimum=10, maximum=120, value=30, step=5, label="API Timeout (seconds)"
                    )
            ...
            with gr.Accordion("🔄 Training Settings", open=False):
                max_iterations = gr.Slider(
                    minimum=1, maximum=5, value=3, step=1, label="Max Iterations"
                )
            ...
            with gr.Tabs():
                with gr.Tab("📋 Labeled Dataset"):
                    dataset_output = gr.Dataframe(
                        label="Labeled Data", interactive=False, wrap=True
                    )

                with gr.Tab("📈 Metadata"):
                    metadata_output = gr.Code(label="Pipeline Metadata", language="json")
    ...
    run_btn.click(
        ...
        outputs=[summary_output, dataset_output, metadata_output, download_output],
    )
...
def main():
    ...
    try:
        ...
        app.launch(share=True, show_error=True, inbrowser=True)
