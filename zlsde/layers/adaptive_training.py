...
from zlsde.models.data_models import LabeledDataItem, PipelineConfig
...
class AdaptiveSelfTrainer:
    ...
    def train_and_refine(
        ...
        threshold = (
            float(np.percentile(confidences[labels >= 0], 100 - percentile))
            if np.any(labels >= 0)
            else 0.0
        )
    ...
    @staticmethod
    def _augment_by_weight(X: np.ndarray, y: np.ndarray, weights: np.ndarray, max_factor: int = 3):
        """Approximate sample weighting by repeating high-confidence rows."""
        # Scale weights to repeat counts in [1, max_factor]
        repeat_counts = np.clip(np.round(weights * max_factor).astype(int), 1, max_factor)
