...
class EmbeddingFusionEngine:
    ...
    def fuse(
        ...
        logger.info(f"Embedding fusion: {len(all_embs)} models -> dim {fused.shape[1]}")
        return fused
    ...
    def _resolve_weights(self, n_models: int) -> List[float]:
        ...
        if self.fusion_weights and len(self.fusion_weights) == n_models:
            ...
            return (
                [w / total for w in self.fusion_weights]
                if total > 0
    ...
    @staticmethod
    def _weighted_concat(embedding_sets: List[np.ndarray], weights: List[float]) -> np.ndarray:
