...
class RepresentationEngine:
    ...
    def __init__(self, modality: str, model_name: str, device: str = "cpu", seed: Optional[int] = None):
        ...
        logger.info(f"RepresentationEngine initialized: modality={modality}, "
    ...
    def reduce_dimensions(self, embeddings: np.ndarray, n_components: int = 50, 
        ...
        if n_samples < n_components:
            raise ValueError(
                f"n_samples ({n_samples}) must be >= n_components ({n_components})"
            )
        ...
        # Validate output
        assert reduced_embeddings.shape == (n_samples, n_components), "Shape mismatch after reduction"
