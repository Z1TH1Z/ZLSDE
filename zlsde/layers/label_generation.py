...
from zlsde.models.data_models import Label, RawDataItem
...
class PseudoLabelGenerator:
    ...
    def generate_labels(self, clusters: Dict[int, List[RawDataItem]]) -> Dict[int, Label]:
        ...
        for cluster_id, items in clusters.items():
            ...
            try:
                ...
                label_text = self.provider_manager.generate_label(prompt, max_tokens=10)
    ...
    def _select_representatives(
        self, cluster_items: List[RawDataItem], k: int = 5
    ) -> List[RawDataItem]:
        """
    ...
    def _create_prompt(self, representatives: List[RawDataItem]) -> str:
        ...
        prompt = f"""What is the single topic or domain that all these text samples share?

Samples:
{samples_text}

Reply with ONLY a 1-3 word category label. No explanation, no punctuation, no sentences. Examples of good labels: "astrophysics", "marine biology", "machine learning", "cooking recipes".
...
Label:"""
    ...
    def _validate_label(self, label: str) -> str:
        ...
        if word_count > 3:
            logger.warning(
                f"Generated label exceeds 3 words ({word_count}): '{label}' - trimming to first word"
            )
