...
class SelfTrainingLoop:
    ...
    def train_classifier(
        self, embeddings: np.ndarray, labels: np.ndarray, confidence_threshold: float = 0.8
    ):
        """
    ...
    def compute_stability(self, old_labels: np.ndarray, new_labels: np.ndarray) -> float:
        ...
        logger.info(
            f"Label flip rate: {flip_rate:.2%} ({n_flips}/{len(old_labels)} labels changed)"
        )
    ...
    def check_convergence(self, metrics_history: List[IterationMetrics]) -> bool:
        ...
        if converged:
            ...
            logger.info(
                f"Not converged - Label stable: {label_stable}, "
