...
from zlsde.models.data_models import (
    PipelineConfig, RawDataItem, Label, TaxonomyNode, TaxonomyTree
)
...
class TaxonomyDiscoveryEngine:
    ...
    def discover(
        ...
        logger.info(
            f"Taxonomy discovered: {total_nodes} nodes, max depth {max_depth}"
        )
        return tree
    ...
    def _max_depth_of(nodes: List[TaxonomyNode]) -> int:
        ...
        return max(
            n.level
            if not n.children
            else TaxonomyDiscoveryEngine._max_depth_of(n.children)
            for n in nodes
        )
