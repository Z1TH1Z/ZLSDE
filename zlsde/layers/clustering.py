...
class ClusteringEngine:
    ...
    def __init__(self, config):
        ...
        logger.info(f"ClusteringEngine initialized: method={config.clustering_method}, "
    ...
    def cluster(self, embeddings: np.ndarray, method: str = "auto") -> ClusterResult:
        ...
        if method == "auto":
            ...
            try:
                ...
                logger.info(f"HDBSCAN: {hdbscan_result.n_clusters} clusters, "
            ...
            for k in [estimated_k, max(2, estimated_k - 1), estimated_k + 1]:
                if k >= 2 and k < n_samples:
                    try:
                        ...
                        logger.info(f"KMeans (k={k}): silhouette={kmeans_result.silhouette_score:.3f}")
                    except Exception as e:
                        logger.warning(f"KMeans with k={k} failed: {e}")
            
            # Try Spectral clustering with estimated k
            if estimated_k >= 2 and estimated_k < n_samples:
                try:
                    logger.info(f"Trying Spectral clustering with k={estimated_k}...")
                    spectral_result = self._cluster_spectral(embeddings, n_clusters=estimated_k)
                    results.append(spectral_result)
                    logger.info(f"Spectral (k={estimated_k}): silhouette={spectral_result.silhouette_score:.3f}")
                except Exception as e:
            ...
            logger.info(f"Selected {best_result.method_used} with silhouette={best_result.silhouette_score:.3f}")
            ...
            k = self._estimate_optimal_k(embeddings)
    ...
    def compute_metrics(self, embeddings: np.ndarray, labels: np.ndarray) -> dict:
        ...
        if len(embeddings) != len(labels):
            raise ValueError(f"embeddings and labels length mismatch: {len(embeddings)} vs {len(labels)}")
