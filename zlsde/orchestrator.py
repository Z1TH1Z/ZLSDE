...
class PipelineOrchestrator:
    ...
    def run(self) -> PipelineResult:
        ...
        try:
            ...
            if self.config.use_dimensionality_reduction:
                ...
                embeddings = self.embedder.reduce_dimensions(
                    embeddings, self.config.n_components
                )
            ...
            for iteration in range(self.config.max_iterations):
                ...
                if (is_last or converged) and self.taxonomy_engine:
                    ...
                    taxonomy_tree = self.taxonomy_engine.discover(
                        embeddings, cluster_arr, pseudo_labels,
    ...
    def _execute_iteration(
        ...
        cluster_result = self.clusterer.cluster(
            embeddings, method=self.config.clustering_method
        )
        logger.info(
        ...
        if iteration < self.config.max_iterations - 1:
            ...
            try:
                if self.adaptive_trainer.enabled:
                    ...
                    current_cluster_labels = np.array(
                        [item.cluster_id for item in labeled_data]
                    )
                    ...
                    refined_labels = self.self_trainer.refine_labels(
                ...
                logger.warning(
                    f"Self-training failed: {e}. Continuing with current labels."
                )
    ...
    def _compute_iteration_metrics(
        ...
        if previous_labels is not None:
            ...
            label_flip_rate = self.self_trainer.compute_stability(
